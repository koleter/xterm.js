...
function testEvaluateKeyboardEvent(partialEvent, partialOptions = {}) {
    ...
    return (0, Keyboard_1.evaluateKeyboardEvent)(event, options.applicationCursorMode, options.isMac, options.macOptionIsMeta);
}
describe('Keyboard', () => {
    describe('evaluateKeyEscapeSequence', () => {
        ...
        it('should return \\x1b\\r for alt+enter', () => {
            chai_1.assert.equal(testEvaluateKeyboardEvent({ altKey: true, keyCode: 13 }).key, '\x1b\r');
        });
        it('should return \\x1b\\x1b for alt+esc', () => {
            chai_1.assert.equal(testEvaluateKeyboardEvent({ altKey: true, keyCode: 27 }).key, '\x1b\x1b');
        });
        ...
        it('should return \\x08 for ctrl+backspace', () => {
            chai_1.assert.equal(testEvaluateKeyboardEvent({ ctrlKey: true, keyCode: 8 }).key, '\x08');
        });
        it('should return \\x1b\\x7f for alt+backspace', () => {
            chai_1.assert.equal(testEvaluateKeyboardEvent({ altKey: true, keyCode: 8 }).key, '\x1b\x7f');
        });
        it('should return \\x1b\\x08 for ctrl+alt+backspace', () => {
            chai_1.assert.equal(testEvaluateKeyboardEvent({ ctrlKey: true, altKey: true, keyCode: 8 }).key, '\x1b\x08');
        });
        it('should return \\x1b[3;2~ for shift+delete', () => {
            chai_1.assert.equal(testEvaluateKeyboardEvent({ shiftKey: true, keyCode: 46 }).key, '\x1b[3;2~');
        });
        ...
        describe('On non-macOS platforms', () => {
            ...
            it('should return \\x1b[5D for alt+up', () => {
                chai_1.assert.equal(testEvaluateKeyboardEvent({ altKey: true, keyCode: 38 }, { isMac: false }).key, '\x1b[1;5A');
            });
            it('should return \\x1b[5C for alt+down', () => {
                chai_1.assert.equal(testEvaluateKeyboardEvent({ altKey: true, keyCode: 40 }, { isMac: false }).key, '\x1b[1;5B');
            });
            ...
            it('should return \\x1b\\x20 for alt+space', () => {
                chai_1.assert.equal(testEvaluateKeyboardEvent({ altKey: true, keyCode: 32 }, { isMac: false }).key, '\x1b\x20');
            });
            it('should return \\x1b\\x00 for ctrl+alt+space', () => {
                chai_1.assert.equal(testEvaluateKeyboardEvent({ altKey: true, ctrlKey: true, keyCode: 32 }, { isMac: false }).key, '\x1b\x00');
            });
        });
        describe('On macOS platforms', () => {
            ...
            it('should return \\x1bb for alt+up', () => {
                chai_1.assert.equal(testEvaluateKeyboardEvent({ altKey: true, keyCode: 38 }, { isMac: true }).key, '\x1b[1;3A');
            });
            it('should return \\x1bf for alt+down', () => {
                chai_1.assert.equal(testEvaluateKeyboardEvent({ altKey: true, keyCode: 40 }, { isMac: true }).key, '\x1b[1;3B');
            });
            it('should return undefined for alt+a', () => {
                chai_1.assert.strictEqual(testEvaluateKeyboardEvent({ altKey: true, keyCode: 65 }, { isMac: true }).key, undefined);
            });
        });
        describe('with macOptionIsMeta', () => {
            ...
            it('should return \\x1b\\x1b for alt+enter', () => {
                chai_1.assert.equal(testEvaluateKeyboardEvent({ altKey: true, keyCode: 13 }, { isMac: true, macOptionIsMeta: true }).key, '\x1b\r');
            });
        });
        ...
        it('should return proper sequence for ctrl+@', () => {
            chai_1.assert.equal(testEvaluateKeyboardEvent({ ctrlKey: true, shiftKey: true, keyCode: 50, key: '@' }).key, '\x00');
        });
    });
});
