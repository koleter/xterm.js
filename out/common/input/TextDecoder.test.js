...
describe('text encodings', () => {
    it('stringFromCodePoint/utf32ToString', () => {
        ...
        for (let i = 0; i < s.length; ++i) {
            ...
            chai_1.assert.equal((0, TextDecoder_1.stringFromCodePoint)(data[i]), s[i]);
        }
        chai_1.assert.equal((0, TextDecoder_1.utf32ToString)(data), s);
    });
    describe('StringToUtf32 decoder', () => {
            ...
            for (let min = 0; min < 65535; min += BATCH_SIZE) {
                ...
                it(`${formatRange(min, max)}`, () => {
                    ...
                    for (let i = min; i < max; ++i) {
                        ...
                        chai_1.assert.equal(length, 1);
                        chai_1.assert.equal(target[0], i);
                        chai_1.assert.equal((0, TextDecoder_1.utf32ToString)(target, 0, length), String.fromCharCode(i));
                        decoder.clear();
                    }
                });
            }
            for (let min = 65536; min < 0x10FFFF; min += BATCH_SIZE) {
                ...
                it(`${formatRange(min, max)} (surrogates)`, () => {
                    ...
                    for (let i = min; i < max; ++i) {
                        ...
                        chai_1.assert.equal(length, 1);
                        chai_1.assert.equal(target[0], i);
                        chai_1.assert.equal((0, TextDecoder_1.utf32ToString)(target, 0, length), s);
                        decoder.clear();
                    }
                });
            }
        ...
        it('test strings', () => {
            ...
            for (let i = 0; i < TEST_STRINGS.length; ++i) {
                ...
                chai_1.assert.equal(toString(target, length), TEST_STRINGS[i]);
                decoder.clear();
            }
        });
    ...
    describe('Utf8ToUtf32 decoder', () => {
        describe('full codepoint test', () => {
            for (let min = 0; min < 65535; min += BATCH_SIZE) {
                ...
                it(`${formatRange(min, max)} (1/2/3 byte sequences)`, () => {
                    ...
                    for (let i = min; i < max; ++i) {
                        ...
                        const utf8Data = fromByteString((0, utf8_1.encode)(String.fromCharCode(i)));
                        const length = decoder.decode(utf8Data, target);
                        chai_1.assert.equal(length, 1);
                        chai_1.assert.equal(toString(target, length), String.fromCharCode(i));
                        decoder.clear();
                    }
                });
            }
            for (let minRaw = 60000; minRaw < 0x10FFFF; minRaw += BATCH_SIZE) {
                ...
                it(`${formatRange(min, max)} (4 byte sequences)`, function () {
                    ...
                    const target = new Uint32Array(5);
                    for (let i = min; i < max; ++i) {
                        const utf8Data = fromByteString((0, utf8_1.encode)((0, TextDecoder_1.stringFromCodePoint)(i)));
                        const length = decoder.decode(utf8Data, target);
                        chai_1.assert.equal(length, 1);
                        chai_1.assert.equal(target[0], i);
                        decoder.clear();
            ...
            it('0xFEFF(BOM)', () => {
                const decoder = new TextDecoder_1.Utf8ToUtf32();
                const target = new Uint32Array(5);
                const utf8Data = fromByteString((0, utf8_1.encode)(String.fromCharCode(0xFEFF)));
                const length = decoder.decode(utf8Data, target);
                chai_1.assert.equal(length, 0);
                decoder.clear();
            });
        ...
        it('test strings', () => {
            ...
            const target = new Uint32Array(500);
            for (let i = 0; i < TEST_STRINGS.length; ++i) {
                const utf8Data = fromByteString((0, utf8_1.encode)(TEST_STRINGS[i]));
                const length = decoder.decode(utf8Data, target);
                chai_1.assert.equal(toString(target, length), TEST_STRINGS[i]);
                decoder.clear();
            }
        ...
        describe('stream handling', () => {
            it('2 byte sequences - advance by 1', () => {
                ...
                (0, chai_1.assert)(decoded, 'ÄÖÜßöäü');
            });
            it('2/3 byte sequences - advance by 1', () => {
                ...
                (0, chai_1.assert)(decoded, 'Ä€Ö€Ü€ß€ö€ä€ü');
            });
            ...
            it('test break after 3 bytes - issue #2495', () => {
                ...
                chai_1.assert.equal(written, 0);
                written = decoder.decode(utf8Data.slice(3), target);
                chai_1.assert.equal(written, 1);
                (0, chai_1.assert)(toString(target, written), '𠜎');
            });
        });
    });
});
